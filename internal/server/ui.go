...
async function load(){var r=await fetch(A+'/deployments').then(function(r){return r.json()});items=r.deployments||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.service||'').toLowerCase().includes(q)||(i.version||'').toLowerCase().includes(q)||(i.environment||'').toLowerCase().includes(q)||(i.strategy||'').toLowerCase().includes(q)});
if(!f.length){document.getElementById('list').innerHTML='<div class="empty">No items found.</div>';return;}
var h='';f.forEach(function(i){
h+='<div class="item"><div class="item-top"><div style="flex:1"><div class="item-title">'+esc(i.service)+'</div>';
...
if(i.environment)h+='<span>'+esc(i.environment)+'</span>';
if(i.strategy)h+='<span>'+esc(i.strategy)+'</span>';
if(i.active_slot)h+='<span>'+esc(i.active_slot)+'</span>';
if(i.status)h+='<span class="badge">'+esc(i.status)+'</span>';
if(i.deployed_by)h+='<span>'+esc(i.deployed_by)+'</span>';
if(i.deployed_at)h+='<span>'+esc(i.deployed_at)+'</span>';
h+='<span>'+ft(i.created_at)+'</span></div>';

h+='</div>';});document.getElementById('list').innerHTML=h;}
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/deployments/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{service:"",version:"",environment:"",strategy:"",active_slot:"",status:"",deployed_by:"",deployed_at:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' SWITCHMAN</h2>';
h+='<div class="fr"><label>Service *</label><input id="f-service" value="'+esc(i.service)+'"></div>';
h+='<div class="fr"><label>Version</label><input id="f-version" value="'+esc(i.version)+'"></div>';
h+='<div class="fr"><label>Environment</label><input id="f-environment" value="'+esc(i.environment)+'"></div>';
h+='<div class="fr"><label>Strategy</label><input id="f-strategy" value="'+esc(i.strategy)+'"></div>';
h+='<div class="fr"><label>Active Slot</label><input id="f-active-slot" value="'+esc(i.active_slot)+'"></div>';
h+='<div class="fr"><label>Status</label><input id="f-status" value="'+esc(i.status)+'"></div>';
h+='<div class="fr"><label>Deployed By</label><input id="f-deployed-by" value="'+esc(i.deployed_by)+'"></div>';
h+='<div class="fr"><label>Deployed At</label><input id="f-deployed-at" value="'+esc(i.deployed_at)+'"></div>';
...
async function submit(){var body={service:document.getElementById('f-service').value.trim(),version:document.getElementById('f-version').value.trim(),environment:document.getElementById('f-environment').value.trim(),strategy:document.getElementById('f-strategy').value.trim(),active_slot:document.getElementById('f-active-slot').value.trim(),status:document.getElementById('f-status').value.trim(),deployed_by:document.getElementById('f-deployed-by').value.trim(),deployed_at:document.getElementById('f-deployed-at').value.trim()};
if(editId){await fetch(A+'/deployments/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/deployments',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
