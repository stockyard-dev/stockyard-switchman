package server
import("encoding/json";"net/http";"github.com/stockyard-dev/stockyard-switchman/internal/store")
...
func(s *Server)routes(){
    s.mux.HandleFunc("GET /health",func(w http.ResponseWriter,r *http.Request){writeJSON(w,200,map[string]string{"status":"ok","service":"stockyard-switchman"})})
    s.mux.HandleFunc("GET /api/stats",s.handleOverview)
    s.mux.HandleFunc("GET /api/services",s.handleList)
    s.mux.HandleFunc("POST /api/services",s.handleCreate)
    s.mux.HandleFunc("DELETE /api/services/{id}",s.handleDelete)
    s.mux.HandleFunc("POST /api/services/{id}/cutover",s.handleCutover)
    s.mux.HandleFunc("POST /api/services/{id}/split",s.handleSplit)
